...
public class ChatPeer extends Peer {
		...
		try { 
			...
			if (msgType.equals(ChatMessage.CHAT_MESSAGE_TYPE)) {
				...
				if (peerDescriptor.getKey().equals(destination)) {
					System.out.println("A message for us :) Received at " + peerDescriptor.getKey());					
				...
				} else if (peerDescriptor.getKey().equals(senderId)){
					// Undelivered message, drops it
					System.out.println("A message from us :( Received at " + peerDescriptor.getKey());
					for (ChatPeerListener listener : listeners) {
						listener.undelivered(destination, message);
					}
				} else {
					System.out.println("A message to pass through");
	...
	public String toString() {
		return peerDescriptor.getKey() + " @ " + peerDescriptor.getAddress();
	}
}