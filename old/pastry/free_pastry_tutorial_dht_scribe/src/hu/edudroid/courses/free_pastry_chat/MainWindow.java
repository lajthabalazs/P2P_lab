...
public class MainWindow extends JFrame implements ActionListener {
	...
	/**
	 * Sends a message to a peer that the name will be looked up.
	 */
	private void sendMessageToPeer() {
		String remoteName = (String)JOptionPane.showInputDialog(this, "Remote peer's name", "Find peer", JOptionPane.PLAIN_MESSAGE, null, null,"");;
		peer.findPeer(remoteName, new Continuation<PastContent, Exception>(){

			@Override
			public void receiveException(Exception e) {
				JOptionPane.showMessageDialog(MainWindow.this,"Peer lookup error" + e.getMessage(), "Peer lookup error", JOptionPane.ERROR_MESSAGE);
			}

			@Override
			public void receiveResult(PastContent content) {
				if (content != null) {
					JOptionPane.showMessageDialog(MainWindow.this,"Name: " + ((UserProfile)content).name + " @ " +  ((UserProfile)content).peerId, "Peer found", JOptionPane.INFORMATION_MESSAGE);
					MainWindow.this.peer.converse(((UserProfile)content).peerId, null);
				} else {
					JOptionPane.showMessageDialog(MainWindow.this,"Peer not found", "Peer lookup failed", JOptionPane.ERROR_MESSAGE);					
				}
			}
		});
	}
	...
	private void joinTopic() {
		if (peer != null) {
			String topicName = (String)JOptionPane.showInputDialog(this, " Name of the topic:","Join a topic", JOptionPane.PLAIN_MESSAGE, null, null,"");
			try {
				peer.joinTopic(topicName);
			} catch (Exception e) {
				JOptionPane.showMessageDialog(this,"Subscription error: " + e.getMessage(), "Subscription error", JOptionPane.ERROR_MESSAGE);
				e.printStackTrace();
			}
		}
	}
